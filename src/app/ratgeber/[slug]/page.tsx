...
export async function generateStaticParams() {
  const slugs = getAllSlugs("ratgeber");
  return slugs.map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params;
  const post = getMDXBySlug("ratgeber", slug);
  ...
  return {
    ...
    alternates: {
      canonical: `https://findius.io/ratgeber/${slug}`,
    },
    openGraph: {
      title: frontmatter.title,
      description: frontmatter.description,
      url: `https://findius.io/ratgeber/${slug}`,
      siteName: "findius",
      type: "article",
      locale: "de_DE",
...
export default async function RatgeberPage({ params }: PageProps) {
  const { slug } = await params;
  const post = getMDXBySlug("ratgeber", slug);
  ...
  const articleJsonLd = {
    ...
    mainEntityOfPage: `https://findius.io/ratgeber/${slug}`,
  };
